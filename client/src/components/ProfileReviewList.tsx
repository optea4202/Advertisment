...
import { type UserReview } from '../api/userReviews.js';
...
interface ProfileReviewListProps {
  ...
  loading: boolean;
  error: Error | null;
  onDeleteReview?: (reviewId: number, reviewerName: string) => void;
}
  ...
  if (error) {
    return (
      <div className="p-md bg-error-container text-on-error-container rounded-lg border border-error/10 text-body-sm text-center">
        Failed to load profile reviews. Please refresh the page to try again.
      </div>
    );
  }
      ...
      <div className="py-xl flex flex-col items-center justify-center text-center gap-sm">
        ...
        <div>
          ...
          <p className="font-body-sm text-body-sm text-secondary mt-xs max-w-xs">
            Be the first to review this member's profile.
          </p>
        </div>
      </div>
    );
          ...
          >
            ...
            <div className="flex justify-between items-start">
              ...
              <div className="flex items-center gap-sm">
                ...
                {/* Delete button (only show for review owner or admins) */}
                {(user?.id === review.reviewer_id || user?.is_admin) && onDeleteReview && (
                  <button
                    onClick={() => onDeleteReview(review.id, review.reviewer_name || 'Anonymous')}
