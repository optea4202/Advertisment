...
export const SignUpPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err.errors?.[0]?.message || 'Failed to sign up. Please try again.');
    } finally {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === 'complete') {
        ...
        navigate('/setup'); // First-time setup
      } else {
        console.warn('Unhandled verification status:', result.status);
        setErrorMsg('Verification could not be completed.');
      }
    } catch (err: any) {
      console.error('Verification error:', err);
      setErrorMsg(err.errors?.[0]?.message || 'Incorrect verification code.');
    } finally {
      setIsSubmitting(false);
    }
  };
          ...
          <div className="flex flex-col gap-xs">
            ...
            <p className="font-body-md text-body-md text-secondary">
              {pendingVerification 
                ...
                : 'Enter your details to register and start advertising.'}
            </p>
          </div>
