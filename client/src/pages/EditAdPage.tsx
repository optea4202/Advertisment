...
export const EditAdPage: React.FC = () => {
  ...
  // Visual preview helper list mapping index -> source (either URL string or File objectURL)
  interface PreviewItem {
    type: 'existing' | 'new';
    src: string;
    index: number; // reference index in respective arrays
  }
  ...
  useEffect(() => {
    if (isNaN(adId)) {
      setErrorMsg('Invalid advertisement ID.');
      setIsLoading(false);
      return;
    }
    ...
    const fetchAdDetails = async () => {
      try {
        ...
        const initialPreviews: PreviewItem[] = urls.map((url, index) => ({
          type: 'existing',
          src: url,
          index
        }));
        ...
        setErrorMsg('Failed to load advertisement data. It may have been deleted or you do not have access.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const processFiles = (files: File[]) => {
    ...
    if (totalCount > 5) {
      setErrorMsg('An advertisement may have a maximum of 5 images.');
      return;
    }
    ...
    // Add files to state
    const updatedNewFiles = [...newFiles, ...imageFiles];
    setNewFiles(updatedNewFiles);

    // Re-generate previews
    rebuildPreviews(existingImages, updatedNewFiles);
  };
  ...
  const removeImage = (previewIndex: number) => {
    ...
    if (target.type === 'existing') {
      updatedExisting = existingImages.filter((_, i) => i !== target.index);
      setExistingImages(updatedExisting);
    } else {
      updatedNewFiles = newFiles.filter((_, i) => i !== target.index);
      setNewFiles(updatedNewFiles);
    }

    rebuildPreviews(updatedExisting, updatedNewFiles);
  };

  const rebuildPreviews = (existingList: string[], newList: File[]) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Ad updated successfully! Redirecting to dashboard...');
      navigate('/dashboard');
    } catch (err: any) {
      console.error('Error updating advertisement:', err);
      const apiError = err.response?.data?.error?.message || 'Failed to update advertisement. Please try again.';
    ...
    <div className="bg-surface text-on-surface min-h-screen flex flex-col">
      ...
      <main className="flex-grow w-full max-w-container-max mx-auto px-md md:px-xl py-xl md:py-xxl flex flex-col gap-xl">
        ...
        <div className="flex flex-col gap-xs">
          <h1 className="font-headline-lg text-headline-lg text-on-surface tracking-tight">Edit Advertisement</h1>
          <p className="font-body-md text-body-md text-secondary">Modify the details of your advertisement listing below.</p>
        </div>
        ...
        {isLoading && (
          <div className="flex flex-col items-center justify-center py-xxl">
            ...
            <p className="font-body-md text-body-md text-secondary mt-md">Loading advertisement details...</p>
          </div>
        )}
                ...
                {previews.length > 0 && (
                  <div className="grid grid-cols-4 gap-sm mt-sm">
                    {previews.map((item, index) => (
                      ...
                      >
                        <img src={item.src} alt="Ad preview" className="w-full h-full object-cover" />
