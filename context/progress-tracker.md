...
- Updated admin layout and navigation header: removed "Home", "My Profile", "About", "Inbox", and "Post Ad" from the admin user view; integrated the admin moderation tabs ("Advertisements", "Reviews", "Users", "Reports") directly as a left vertical sidebar (from top to bottom) for admin users on desktop, while retaining the standard horizontal top navbar layout for regular users and guests; removed the "Admin" text link; and implemented automatic redirects to `/admin` for administrators attempting to access user-specific pages.
