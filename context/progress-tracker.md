...
- Updated admin layout and navigation header: removed "Home", "My Profile", "About", "Inbox", and "Post Ad" from the admin user view; integrated the admin moderation tabs ("Advertisements", "Reviews", "Users", "Reports") directly as the main navigation bar in the header aligned to the left side next to the brand logo; removed the "Admin" text link; and implemented automatic redirects to `/admin` for administrators attempting to access user-specific pages.
